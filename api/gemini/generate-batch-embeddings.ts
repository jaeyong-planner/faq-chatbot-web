...
import { generateBatchEmbeddings } from "./_lib/geminiClient.js";
import {
  extractToken,
  validateToken,
  parseRequestBody,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const embeddings = await Promise.race([
      generateBatchEmbeddings(body.texts),
      timeoutPromise,
    ]);
    ...
    return res
      .status(500)
      .json({ error: error.message || "Failed to generate batch embeddings" });
  }
}
