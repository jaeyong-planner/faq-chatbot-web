...
const ChunkManagement: React.FC = () => {
  ...
  const filteredChunks = chunks.filter(chunk => {
    ...
    return (
      ...
      chunk.metadata?.keywords?.some(kw => kw.toLowerCase().includes(searchLower))
    );
  });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="text-sm text-gray-600">
              총 <span className="font-semibold text-blue-600">{documents.length}</span>개 문서
            </div>
            {selectedDocumentId && (
              <div className="text-sm text-gray-600">
                <span className="font-semibold text-green-600">{chunks.length}</span>개 청크
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900 mb-3">문서 목록</h2>
            <input
              type="text"
              placeholder="문서 검색..."
                ...
                {documents.map((doc) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${
                      selectedDocumentId === doc.id ? 'bg-blue-50 border-l-4 border-blue-600' : ''
                    }`}
                  >
                ...
                <div className="divide-y divide-gray-200">
                  {filteredChunks.map((chunk) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${
                        selectedChunk?.id === chunk.id ? 'bg-blue-50 border-l-4 border-blue-600' : ''
                      }`}
                    >
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2 text-xs text-gray-500">
                          ...
                          <span className="inline-flex items-center px-2 py-0.5 rounded bg-blue-100 text-blue-700">
                            {chunk.metadata?.pageLabel || `${chunk.pageNumber}페이지`}
                          </span>
                          {chunk.metadata?.importance && (
                            <span className={`inline-flex items-center px-2 py-0.5 rounded ${
                              ...
                              {chunk.metadata.importance === 'high' ? '높음' : chunk.metadata.importance === 'medium' ? '보통' : '낮음'}
                            </span>
                          )}
                        </div>
                        {chunk.metadata?.keywords && chunk.metadata.keywords.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {chunk.metadata.keywords.slice(0, 3).map((keyword, idx) => (
                              <span key={idx} className="text-xs px-1.5 py-0.5 bg-purple-50 text-purple-700 rounded">
                                {keyword}
                              </span>
                            ))}
                          </div>
          ...
          {selectedChunk ? (
            <>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h2 className="text-xl font-bold text-gray-900 mb-2">
                      {selectedChunk.metadata?.title || `청크 #${selectedChunk.chunkIndex + 1}`}
                    </h2>
                    <div className="flex items-center space-x-3 text-sm text-gray-600">
                      <span className="inline-flex items-center">
                        ...
                        {selectedChunk.metadata?.pageLabel || `${selectedChunk.pageNumber}페이지`}
                      </span>
                      <span>•</span>
                ...
                {selectedChunk.metadata && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedChunk.metadata.importance && (
                      <div>
                        <p className="text-xs font-semibold text-gray-700 mb-1">중요도</p>
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                          ...
                          {selectedChunk.metadata.importance === 'high' ? '높음' : selectedChunk.metadata.importance === 'medium' ? '보통' : '낮음'}
                        </span>
                      </div>
                    )}
                    {selectedChunk.metadata.chunkType && (
                      <div>
                        <p className="text-xs font-semibold text-gray-700 mb-1">청크 유형</p>
                    ...
                    {selectedChunk.metadata.keywords && selectedChunk.metadata.keywords.length > 0 && (
                      <div className="col-span-2">
                        <p className="text-xs font-semibold text-gray-700 mb-2">핵심 키워드</p>
                        <div className="flex flex-wrap gap-2">
                          {selectedChunk.metadata.keywords.map((keyword, idx) => (
                    ...
                    {selectedChunk.metadata.semanticKeywords && selectedChunk.metadata.semanticKeywords.length > 0 && (
                      <div className="col-span-2">
                        <p className="text-xs font-semibold text-gray-700 mb-2">의미적 키워드</p>
                        <div className="flex flex-wrap gap-2">
                          {selectedChunk.metadata.semanticKeywords.map((keyword, idx) => (
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900">OCR 원문</h3>
                  <button
                    onClick={() => {
                ...
                {/* Context Information */}
                {(selectedChunk.metadata?.contextBefore || selectedChunk.metadata?.contextAfter) && (
                  <div className="mt-6 space-y-4">
                    <h3 className="text-lg font-semibold text-gray-900">문맥 정보</h3>
                    {selectedChunk.metadata.contextBefore && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p className="text-xs font-semibold text-blue-900 mb-2">이전 문맥</p>
                        <p className="text-sm text-blue-800">{selectedChunk.metadata.contextBefore}</p>
                      </div>
                    )}
                    {selectedChunk.metadata.contextAfter && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        <p className="text-xs font-semibold text-green-900 mb-2">다음 문맥</p>
                        <p className="text-sm text-green-800">{selectedChunk.metadata.contextAfter}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 flex items-center justify-center text-gray-500">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">왼쪽 목록에서 청크를 클릭하면 OCR 원문을 확인할 수 있습니다</p>
              </div>
            </div>
          ) : (
            <div className="flex-1 flex items-center justify-center text-gray-500">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">왼쪽 목록에서 문서를 선택하면 청크 목록을 확인할 수 있습니다</p>
              </div>
            </div>
          )}
        </div>
