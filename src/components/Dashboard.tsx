...
const CATEGORY_COLORS = ['bg-blue-500', 'bg-purple-500', 'bg-green-500', 'bg-yellow-500', 'bg-red-500', 'bg-indigo-500'];
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigateToChatLogs, onGoToChatbot }) => {
  ...
  const categories = useMemo(() => {
    const total = metrics.faqCategoryDistribution.reduce((sum, item) => sum + item.count, 0);
    if (total === 0) {
      return [
        { name: '데이터 없음', value: 0, color: 'bg-gray-300', raw: 0 }
      ];
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-medium text-black">{isLoading ? '로딩 중...' : lastUpdatedText}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-black mb-4">카테고리별 질문 분포</h3>
          ...
          {categories.map((category, index) => (
            <div key={index} className="flex items-center justify-between">
              ...
              <div className="flex items-center">
                ...
                <span className="text-sm text-gray-600 w-16 text-right">{category.value}% ({category.raw})</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {recentConversations.length === 0 ? (
          ...
          <div className="space-y-4">
            {recentConversations.map((conv) => (
              <div key={conv.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors duration-200">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                      사용자
                    </span>
                    {conv.confidence !== null && conv.confidence !== undefined && (
                ...
                <p className="text-sm text-black mb-3 line-clamp-2">{conv.userMessage}</p>
                <div className="bg-gray-50 rounded-lg p-3">
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                      챗봇
                    </span>
                    {conv.satisfaction !== null && conv.satisfaction !== undefined && (
                      <div className="flex items-center">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg
                            key={star}
                  ...
                  <p className="text-sm text-gray-700 line-clamp-3">{conv.botResponse}</p>
                </div>
              </div>
            ))}
          </div>
