...
const PDFDocumentViewer: React.FC<PDFDocumentViewerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "ocr" | "content" | "faqs" | "metadata"
  >("overview");
  ...
  const pageContents = useMemo(() => {
    if (document.chunks && document.chunks.length > 0) {
      ...
      return Array.from({ length: totalPages }, (_, index) => {
        ...
        const text = (chunkMap.get(pageNumber) || [])
          .filter(Boolean)
          .join("\n\n")
          .trim();
        return {
  ...
  const searchResults = useMemo(() => {
    ...
    const matchingPages = Array.from(perPageMatches.keys()).sort(
      (a, b) => a - b,
    );
  ...
  const currentPageContent =
    totalPages > 0
      ? pageContents[Math.min(currentPage - 1, totalPages - 1)]
      : null;
  ...
  const tabs = [
    ...
    },
    {
      id: "content",
      name: "내용",
      icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V19a2 2 0 01-2 2z",
    },
    {
      id: "faqs",
      name: "엠브레인Agent관리",
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">
                        {metadata.images?.length || 0}개
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600">그래프</p>
                      <p className="font-medium">
                        {metadata.graphs?.length || 0}개
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600">테이블</p>
                      <p className="font-medium">
                        {metadata.tables?.length || 0}개
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-5">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                ...
                <p className="text-sm text-blue-700 mt-2">
                  PDF에서 추출된 원본 OCR 텍스트입니다. AI 처리나 요약 없이 있는
                  그대로 표시됩니다.
                </p>
              </div>
              ...
              {document.ocrText ? (
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                  ...
                  <p className="text-sm text-yellow-600 mt-2">
                    이 문서는 OCR 처리가 완료되지 않았거나 원본 텍스트가
                    저장되지 않았습니다.
                  </p>
                  <p className="text-xs text-yellow-500 mt-3">
                    참고: 이 기능은 새로 업로드한 문서에만 적용됩니다. 기존
                    문서는 OCR 원본 텍스트가 저장되지 않았을 수 있습니다.
                  </p>
                  {metadata?.textContent && (
                    <div className="mt-4 p-3 bg-yellow-100 rounded-lg text-left">
                      ...
                      <p className="text-xs text-yellow-600">
                        원본 OCR 텍스트는 없지만, AI 처리된 텍스트는 "내용"
                        탭에서 확인할 수 있습니다.
                      </p>
                    </div>
                  )}
              ...
              {document.ocrText && (
                <div className="bg-gray-100 rounded-lg p-4 text-sm text-gray-600">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>
                      원본 OCR 텍스트 길이:{" "}
                      {document.ocrText.length.toLocaleString()}자
                      {document.ocrText.includes("=== 페이지") && (
                        <span className="ml-2">(페이지 구분 마커 포함)</span>
                      )}
                    </span>
                  </div>
                </div>
          ...
          {activeTab === "content" && (
            <div className="space-y-5">
              ...
              {normalizedSearchTerm &&
                searchResults.matchingPages.length > 0 && (
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 min-h-[320px] max-h-[420px] overflow-y-auto">
                {totalPages === 0 && (
                  <div className="text-sm text-gray-500">
                    문서에서 추출된 텍스트가 없습니다. OCR 처리가 완료되었는지
                    확인해주세요.
                  </div>
                )}
                {totalPages > 0 && (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>
                        페이지 {currentPageContent?.pageNumber ?? currentPage} /{" "}
                        {totalPages}
                      </span>
                      {normalizedSearchTerm &&
                        searchResults.perPageMatches.get(
                          currentPageContent?.pageNumber || 0,
                        ) && (
                          <span>
                            현재 페이지 검색 결과{" "}
                            {searchResults.perPageMatches.get(
