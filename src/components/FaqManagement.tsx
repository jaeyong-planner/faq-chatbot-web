...
const FaqManagement: React.FC<FaqManagementProps> = ({
  ...
  const [featuredSettings, setFeaturedSettings] =
    useState<FeaturedSettings>(loadFeaturedSettings);
  const [topQuestions, setTopQuestions] = useState<
    ChatAnalytics["topQuestions"]
  >([]);
  const [isAutoProcessing, setIsAutoProcessing] = useState(false);
  const [isFallbackPanelOpen, setIsFallbackPanelOpen] = useState(false);
  const [fallbackConfig, setFallbackConfig] =
    useState<FallbackMessageConfig>(loadFallbackConfig);
  const [csInfo, setCsInfo] = useState<CustomerServiceInfo>(
  ...
  const handleResetFallback = useCallback(() => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(DEFAULT_CUSTOMER_SERVICE),
    );
  ...
  const applyAutoFeatured = useCallback(async () => {
    ...
    try {
      ...
      await reloadFAQs();
      showToast(
        `주간 인기 질문 기반으로 ${matchedCount}개 FAQ가 자동 설정되었습니다.`,
        "success",
      );
    } catch (error) {
    ...
    try {
      ...
      if (editingFaq) {
        ...
        if (updated) {
          savedFaq = updated;
          setFaqs((prev) =>
            prev.map((faq) => (faq.id === editingFaq.id ? updated : faq)),
          );
        } else {
      ...
      autoEmbeddingService
        .generateAndSaveFAQEmbeddings(savedFaq)
        .catch((error) => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              ...
              <p className="text-sm text-gray-500">
                현재 {featuredFaqs.length}개 등록 · 모드:{" "}
                {featuredSettings.mode === "auto" ? "자동 (주간 인기)" : "수동"}
              </p>
            </div>
          </div>
          <svg
            className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${isFeaturedPanelOpen ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 space-y-4">
              ...
              <div className="bg-yellow-50 rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-yellow-600 mt-1">
                      주간 사용자 질문 빈도를 기준으로 자주 묻는 질문을 자동
                      선정합니다.
                    </p>
                  </div>
                  <button
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const newSettings = {
                        ...featuredSettings,
                        autoCount: Number(e.target.value),
                      };
                      setFeaturedSettings(newSettings);
                      saveFeaturedSettings(newSettings);
                    }}
                ...
                {featuredSettings.lastAutoUpdated && (
                  <p className="text-xs text-yellow-500">
                    마지막 자동 업데이트:{" "}
                    {new Date(featuredSettings.lastAutoUpdated).toLocaleString(
                      "ko-KR",
                    )}
                  </p>
                )}
                ...
                {topQuestions.length > 0 && (
                  <div className="mt-2">
                    <p className="text-xs font-medium text-yellow-700 mb-2">
                      주간 인기 질문 TOP {Math.min(topQuestions.length, 5)}:
                    </p>
                    <div className="space-y-1">
                      {topQuestions.slice(0, 5).map((tq, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between text-xs bg-white rounded px-3 py-1.5"
                        >
                          <span className="text-gray-700 truncate flex-1">
                            {idx + 1}. {tq.question}
                          </span>
                          <span className="text-yellow-600 font-medium ml-2 whitespace-nowrap">
                            {tq.count}회
                          </span>
                        </div>
                      ))}
            ...
            {featuredSettings.mode === "manual" && (
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <p className="text-xs text-blue-600 mt-1">
                  아래 FAQ 목록에서 별(★) 아이콘을 클릭하여 자주 묻는 질문을
                  직접 선택하세요. (최대 4개)
                </p>
              </div>
            )}
            ...
            <div>
              <p className="text-sm font-medium text-gray-700 mb-2">
                현재 등록된 자주 묻는 질문 ({featuredFaqs.length}/4)
              </p>
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              ...
              <p className="text-sm text-gray-500">
                등록된 FAQ가 없는 질문에 대한 안내 메시지를 설정합니다
              </p>
            </div>
          </div>
          <svg
            className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${isFallbackPanelOpen ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
              ...
              {paginatedFaqs.map((faq) => (
                ...
                >
                    ...
                    <div className="max-w-2xl">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        {faq.sourceDocument ? (
                          ...
                          >
                            <svg
                              className="w-3.5 h-3.5 text-gray-500 group-hover:text-blue-600 flex-shrink-0"
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
                            ...
                            <svg
                              className="w-3 h-3 text-gray-400 group-hover:text-blue-600 flex-shrink-0"
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      <div
                        className={`w-2 h-2 rounded-full mr-2 ${faq.isActive ? "bg-green-400" : "bg-red-400"}`}
                      ></div>
                      {faq.isActive ? "활성" : "비활성"}
                    </button>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        <svg
                          className="w-4 h-4"
                          fill={faq.isFeatured ? "currentColor" : "none"}
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
