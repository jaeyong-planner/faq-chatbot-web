...
export const exportAnalyticsToExcel = (
  ...
  const hourlyRows = analytics.hourlyDistribution.map((h) => [
    `${h.hour}시`,
    h.count,
  ]);
  ...
  const topRows = analytics.topQuestions.map((q, i) => [
    i + 1,
    q.question,
    q.count,
    q.category || "",
  ...
  const totalQuestions = analytics.hourlyDistribution.reduce(
    (sum, h) => sum + h.count,
    0,
  );
  const summaryData = [
    ...
    [
      "평균 만족도",
      analytics.satisfactionAverage != null
        ? `${analytics.satisfactionAverage.toFixed(1)}/5`
        : "-",
    ],
    [
      "세션 해결률",
...
export const exportDashboardToExcel = (metrics: DashboardMetrics): void => {
  ...
  const summaryData = [
    ...
    [
      ...
      metrics.lastActivity
        ? new Date(metrics.lastActivity).toLocaleString("ko-KR")
        : "데이터 없음",
    ],
  ];
  ...
  const catRows = metrics.faqCategoryDistribution.map((c) => [
    c.category,
    c.count,
  ]);
