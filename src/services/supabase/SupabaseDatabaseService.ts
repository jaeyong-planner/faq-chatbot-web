...
export class SupabaseDatabaseService {
  ...
  private async safeQuery<T>(operation: string, fn: () => Promise<T>): Promise<T> {
    try {
      return await fn();
    } catch (error) {
  ...
  // Document operations
  async createDocument(document: Omit<PDFDocument, 'id'>): Promise<PDFDocument> {
  ...
  async getDocument(id: number): Promise<PDFDocument | null> {
    return this.safeQuery('getDocument', async () => {
      ...
      return {
        ...doc,
        chunks: doc.pdfChunks && doc.pdfChunks.length > 0 ? doc.pdfChunks : undefined,
  ...
  async updateDocument(id: number, updates: Partial<PDFDocument>): Promise<PDFDocument | null> {
    return this.safeQuery('updateDocument', async () => {
      ...
      if (updates.uploadMode !== undefined) dbUpdates.upload_mode = updates.uploadMode;
      if (updates.metadata !== undefined) dbUpdates.metadata = updates.metadata;
      if (updates.nameEmbedding !== undefined) dbUpdates.name_embedding = updates.nameEmbedding;
  ...
  async updateChunk(id: number, updates: Partial<PDFChunk>): Promise<PDFChunk | null> {
    return this.safeQuery('updateChunk', async () => {
      ...
      if (updates.chunkIndex !== undefined) dbUpdates.chunk_index = updates.chunkIndex;
      if (updates.embeddings !== undefined) dbUpdates.embeddings = updates.embeddings;
      if (updates.metadata !== undefined) dbUpdates.metadata = updates.metadata;
  ...
  async updateFAQ(id: number, updates: Partial<FAQ>): Promise<FAQ | null> {
    return this.safeQuery('updateFAQ', async () => {
      ...
      if (updates.questionEmbedding !== undefined) dbUpdates.question_embedding = updates.questionEmbedding;
      if (updates.answerEmbedding !== undefined) dbUpdates.answer_embedding = updates.answerEmbedding;
      if (updates.isFeatured !== undefined) dbUpdates.is_featured = updates.isFeatured;
      if (updates.featuredAt !== undefined) dbUpdates.featured_at = updates.featuredAt;
      if (updates.sourceChunkIds !== undefined) dbUpdates.source_chunk_ids = updates.sourceChunkIds;
      if (updates.pageReferences !== undefined) dbUpdates.page_references = updates.pageReferences;
      if (updates.documentLink !== undefined) dbUpdates.document_link = updates.documentLink;
      if (updates.semanticKeywords !== undefined) dbUpdates.semantic_keywords = updates.semanticKeywords;
      if (updates.relatedTopics !== undefined) dbUpdates.related_topics = updates.relatedTopics;
      if (updates.confidence !== undefined) dbUpdates.confidence = updates.confidence;
      if (updates.generationSource !== undefined) dbUpdates.generation_source = updates.generationSource;
    ...
    return this.safeQuery('setFAQFeatured', async () => {
      if (isFeatured) {
        ...
        if (count && count >= 4) {
          ...
          if (oldest) {
            console.log(`⚠️ Featured FAQ가 4개를 초과하여 가장 오래된 FAQ (ID: ${oldest.id})를 자동으로 해제합니다.`);
            await supabase
              .from('faqs')
              .update({ is_featured: false, featured_at: null, updated_at: new Date().toISOString() })
              .eq('id', oldest.id);
          }
        }
  ...
  async createChatSession(session: ChatSessionCreateInput): Promise<ChatSession> {
  ...
  async updateChatSession(sessionId: string, updates: ChatSessionUpdateInput): Promise<ChatSession | null> {
  ...
  async createChatMessage(message: ChatLogMessageCreateInput): Promise<ChatLogMessage> {
  ...
  async getChatMessagesBySessionId(sessionId: string): Promise<ChatLogMessage[]> {
  ...
  async getChatAnalytics(period: 'today' | 'week' | 'month' = 'month'): Promise<ChatAnalytics> {
    return this.safeQuery('getChatAnalytics', async () => {
      const { data, error } = await supabase.rpc('get_chat_analytics', { period });
      if (error) throw error;
  ...
  async getStats(): Promise<{
    ...
    return this.safeQuery('getStats', async () => {
      const [docsResult, chunksResult, faqsResult, completedResult] = await Promise.all([
        supabase.from('pdf_documents').select('id', { count: 'exact', head: true }),
        supabase.from('pdf_chunks').select('id', { count: 'exact', head: true }),
        supabase.from('faqs').select('id', { count: 'exact', head: true }).eq('is_active', true),
        supabase.from('pdf_documents').select('id', { count: 'exact', head: true }).eq('status', 'completed')
      ]);
